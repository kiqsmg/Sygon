...
const Navbar = () => {
  ...
  return (
    ...
    <img src={logosygon} alt="Sygon logo with a fingerprint" className="w-[160px] h-[48px]" />
    ...
    <div className="flex-1 flex justify-center">
      <ul className='list-none sm:flex hidden justify-center items-center'>
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`font-inter font-semibold cursor-pointer text-[20px] ${index === navLinks.length - 1 ? 'mr-0' : 'mr-20'} text-black transition duration-200 hover:scale-110`}
          >
            <a href={`#${nav.id}`}>
              {nav.title}
            </a>
